...
#define TRANS_RD_BYTES_NUM                                                    \
  8 /**< amount of bytes read form serial port at once when                   \
       transparent/MSP/LTM is selected */
...
/** Used on the ESP AIR side to keep track and used to fill MAVlink RADIO
 * STATUS msg */
typedef struct {
  int8_t
  ...
  int8_t air_noise_floor; /**<  [dBm] Noise floor on air side. Updated when
                             ESP32 is in ESP-NOW mode and receives packet - Not
  ...
  uint16_t gnd_rx_packets_lost; /**<  Number of ESP-NOW packets the GND station
                                   lost coming from this AIR peer (based on
                                   seq. number) */
