...
/**
 * Based on the system architecture and configured wifi mode the ESP32 may have
 * a different role and system id. Returns the best fitting component ID for
 * the specific role.
...
/**
 * Converts the measured (negative dBm) signal strength to a format the MAVLink
 * RADIO STATUS packet accepts and the GCS likes. If QGroundControl is desired
 * output format it will not convert but send the value as int8. For Mission
 * Planner it converts int8 to uint8. The value represents the absolute(dBm):
 * -54 dBm -> 54
 ...
 * @return Signal strength formatted for QGroundControl (0 to -127 [dBm]) or
 * Mission Planner (0 to 100)
 */
int8_t
db_format_rssi(int8_t signal_strength, int8_t noise_floor)
{
  if(db_param_rssi_dbm.value.db_param_u8.value) {
    // report in [dBm] - no conversion since in Wi-Fi, BLE & ESP-NOW APIs
    // natively report dBm
 ...
 */
uint16_t
db_create_heartbeat(uint8_t *buff, fmav_status_t *fmav_status)
{
...
/**
 * Creates a mavlink PARAM_VALUE message inside the provided buffer using the
 * provided parameter
 *
 ...
 * @param value The value of the parameter to be sent -> will be converted to
 * IEEE 745
 ...
 */
uint16_t
db_get_mavmsg_param_value(uint8_t *buff, fmav_status_t *fmav_status,
                          uint16_t param_index, float_int_union *value,
                          uint8_t type, char *param_id)
{
  fmav_param_value_t fmav_param_value = { .param_value = value->f,
                                          .param_type = type,
  ...
  return fmav_msg_param_value_encode_to_frame_buf(buff,
                                                  db_get_mav_sys_id(),
                                                  db_get_mav_comp_id(),
                                                  &fmav_param_value,
                                                  fmav_status);
...
/**
 * Gets the mavlink parameter value as float_int_union based on the parameter
 * ID from the internal variable. Maps MAVLink parameter names to the internal
 * variable names.
 *
 * @param float_int IEEE 754 storage for the retrieved value
 * @param param_id  Parameter name you want the value of
 * @param param_index Index of the parameter. May be -1 if requested parameter
 * shall be found based on param_id
 * @return MAV_PARAM_TYPE of the parameter. Returns 0 if the parameter was not
 * found
 */
MAV_PARAM_TYPE
db_mav_get_parameter_value(float_int_union *float_int, const char *param_id,
                           const int16_t param_index)
{
  MAV_PARAM_TYPE type = 0;
  if(param_index >= DB_PARAM_MAV_CNT) {
    ESP_LOGE(TAG,
             "Requested mavlink parameter index %i is out of range (0-%i)",
             param_index,
             DB_PARAM_MAV_CNT - 1);
    return 0;
  }
  for(int i = 0; i < sizeof(db_params) / sizeof(db_params[0]); i++) {
    if(strncmp(param_id, (char *)db_params[i]->mav_t.param_name, 16) == 0 ||
       param_index == db_params[i]->mav_t.param_index) {
      ...
      case STRING:
        ESP_LOGE(
          TAG,
          "db_mav_get_parameter_value(): String parameter not supported.");
        break;
      case UINT8:
      ...
      default:
        ESP_LOGE(TAG,
                 "db_mav_get_parameter_value() -> db_parameter.type unknown!");
        break;
      }
    }
    else {
...
/**
 * Writes the parameter received via mavlink PARAM_SET to the internal variable
 * and triggers write to NVS. For some parameters to become effective the ESP32
 * still needs to be rebooted! No string/blob parameters supported for now.
 *
 * @param param_set_payload
 * @return 1 in case of success and 0 in case of failure
 */
bool
db_write_mavlink_parameter(const fmav_param_set_t *param_set_payload)
{
  // BEWARE: ONLY WORKS WITH NUMBERS FOR NOW! - NO SUPPORT FOR STRINGS
  float_int_union float_int; // used to convert from IEEE 754
  float_int.f = param_set_payload
                  ->param_value; // read parameter value into helper structure
  bool success = false;
  for(int i = 0; i < sizeof(db_params) / sizeof(db_params[0]); i++) {
    if(strncmp(param_set_payload->param_id,
               (char *)db_params[i]->mav_t.param_name,
               16) == 0) {
      switch(db_params[i]->type) {
      case STRING:
 ...
 */
void
db_answer_mavlink_cmd_request_message(uint16_t requested_msg_id, uint8_t *buff,
                                      ...
                                      fmav_status_t *status)
{
  ...
  case FASTMAVLINK_MSG_ID_AUTOPILOT_VERSION: {
    ...
    uint16_t len = fmav_msg_command_ack_encode_to_frame_buf(
      buff, db_get_mav_sys_id(), db_get_mav_comp_id(), &a, status);
    ...
    len = fmav_msg_autopilot_version_encode_to_frame_buf(buff,
                                                         db_get_mav_sys_id(),
                                                         db_get_mav_comp_id(),
                                                         &autopilot_version,
                                                         status);
  ...
  default: {
    ESP_LOGW(TAG,
             "Unsupported MavLink requested message: %i - ignoring",
             requested_msg_id);
  } break;
  }
}
...
/**
 * Called when a MSG_ID_COMMAND_LONG was received. Handles the command
 * processing.
 ...
 */
void
db_process_mavlink_command(fmav_command_long_t *the_command,
                           fmav_message_t *the_msg, fmav_status_t *status,
                           ...
                           int *tcp_clients, udp_conn_list_t *udp_conns)
{
  ...
  case MAV_CMD_REQUEST_MESSAGE: {
    ...
    db_answer_mavlink_cmd_request_message(
      req_msg_id, buff, origin, tcp_clients, udp_conns, the_msg, status);
  } break;
  default: {
    fmav_command_ack_t b = { .command = the_command->command,
                             ...
                             .target_component = the_msg->compid };
    uint16_t len = fmav_msg_command_ack_encode_to_frame_buf(
      buff, db_get_mav_sys_id(), db_get_mav_comp_id(), &b, status);
    ESP_LOGW(TAG,
             "Unsupported MavLink command request: %i - ignoring",
             the_command->command);
    db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);
  } break;
  }
 ...
 * Processes Mavlink messages and sends the radio status message and heartbeat
 * to the GCS on every heartbeat received via UART
 *
 * We expect the FC to be connected to serial port when in WiFi-AP or in
 * WiFi-Client Mode. We expect the GCS to be connected to serial port when in
 * AP-LR or ESP-NOW GND mode.
 *
 * @param new_msg Message to process
 * @param tcp_clients List of connected tcp clients
 * @param udp_conns List of connected UDP clients
 * @param fmav_status fastmavlink library parser status - setup once by the
 * parser for a specific link/interface
 * @param origin Indicates from what kind of input/link we received the new
 * message.
 */
void
handle_mavlink_message(fmav_message_t *new_msg, int *tcp_clients,
                       udp_conn_list_t *udp_conns, fmav_status_t *fmav_status,
                       enum DB_MAVLINK_DATA_ORIGIN origin)
{
  static uint8_t buff[296]; // buffer to handle the response messages - no need
                            // to init every time
  switch(new_msg->msgid) {
  case FASTMAVLINK_MSG_ID_HEARTBEAT:
    if(origin == DB_MAVLINK_DATA_ORIGIN_SERIAL) {
      // we only process heartbeats coming from the UART (local device) since
      // we also use it as a trigger to send our heartbeat
      fmav_heartbeat_t payload;
      fmav_msg_heartbeat_decode(&payload, new_msg);
      if(payload.autopilot == MAV_AUTOPILOT_INVALID &&
         payload.type == MAV_TYPE_GCS) {
        ...
        if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_ESPNOW_GND ||
          ...
          // Send heartbeat to GCS: Every ESP32 no matter its role or mode is
          ...
          ESP_LOGW(TAG,
                   "We received a heartbeat from GCS while not being in "
                   "DB_WIFI_MODE_ESPNOW_GND or "
                   "DB_WIFI_MODE_AP_LR mode. Check your configuration! "
                   "AIR-Side ESP32 seems to be "
                   "connected to GCS via UART");
        }
      }
      else if(payload.autopilot != MAV_AUTOPILOT_INVALID &&
              new_msg->compid == MAV_COMP_ID_AUTOPILOT1) {
        ESP_LOGD(TAG,
                 "Got heartbeat from flight controller (sysID: %i)",
                 new_msg->sysid);
        // This means we are connected to the FC since we only parse mavlink on
        // UART and thus only see the device we are connected to via UART
        DB_MAV_SYS_ID = new_msg->sysid;
        // Check if FC is armed and the Wi-Fi switch based on armed status is
        ...
        if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA ||
          ...
          uint16_t len =
            fmav_msg_radio_status_encode_to_frame_buf(buff,
                                                      db_get_mav_sys_id(),
                                                      db_get_mav_comp_id(),
                                                      &payload_r,
                                                      fmav_status);
          db_send_to_all_clients(tcp_clients, udp_conns, buff, len);
                ...
                wifi_sta_list.num > 0) {
          // We assume ESP32 is not used in DB_WIFI_MODE_AP on the ground but
          // only on the drone side
          ...
          //  Easier for UDP since we have a nice list with mac addresses to
          //  use for mapping. Harder for TCP -> no MAC addresses available of
          //  connected clients
          fmav_radio_status_t payload_r = { .fixed = UINT8_MAX,
                                            .noise = UINT8_MAX,
                                            .remnoise = UINT8_MAX,
                                            .remrssi = db_format_rssi(
                                              wifi_sta_list.sta[0].rssi, -88),
                                            .rssi = UINT8_MAX,
                                            .rxerrors = 0,
                                            .txbuf = 0 };
          uint16_t len =
      ...
      uint16_t length = db_create_heartbeat(buff, fmav_status);
      // Send heartbeat to GND clients: Every ESP32 no matter its role or mode
      // is emitting a heartbeat
      db_send_to_all_clients(tcp_clients, udp_conns, buff, length);
    } // do not react to heartbeats received via wireless interface - reaction
      // to serial is sufficient
    break;
  case FASTMAVLINK_MSG_ID_PARAM_REQUEST_LIST: {
    ESP_LOGI(TAG,
             "Received PARAM_REQUEST_LIST msg. Responding with parameters");
    ...
    for(int i = 0; i < sizeof(db_params) / sizeof(db_params[0]); i++) {
      ...
      default:
        ESP_LOGE(
          TAG,
          "db_param_write_all_params_json() -> db_parameter.type unknown!");
        break;
      }
      len = db_get_mavmsg_param_value(buff,
                                      fmav_status,
                                      db_params[i]->mav_t.param_index,
                                      &float_int,
                                      db_params[i]->mav_t.param_type,
                                      (char *)db_params[i]->mav_t.param_name);
      db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);
    }
  } break;
  case FASTMAVLINK_MSG_ID_PARAM_REQUEST_READ: {
    ...
    float_int_union float_int;
    ESP_LOGI(TAG,
             "GCS request reading parameter ID: %s with index %i",
             payload.param_id,
             payload.param_index);
    MAV_PARAM_TYPE type = db_mav_get_parameter_value(
      &float_int, payload.param_id, payload.param_index);
    if(type != 0) {
      uint16_t len = db_get_mavmsg_param_value(buff,
                                               fmav_status,
                                               payload.param_index,
                                               &float_int,
                                               type,
  ...
  case FASTMAVLINK_MSG_ID_PARAM_SET: {
    ...
    ESP_LOGI(
      TAG, "GCS requested setting parameter %s", parame_set_payload.param_id);
    if(db_write_mavlink_parameter(&parame_set_payload)) {
      // Respond with parameter
      float_int_union float_int;
      MAV_PARAM_TYPE type = db_mav_get_parameter_value(
        &float_int, parame_set_payload.param_id, -1);
      if(type != 0) {
        uint16_t len = db_get_mavmsg_param_value(
          buff, fmav_status, 0, &float_int, type, parame_set_payload.param_id);
        db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);
        ...
        ESP_LOGE(
          TAG,
          "Failed to get parameter %s - could not respond with new param",
          parame_set_payload.param_id);
      }
    }
    else {
      ESP_LOGE(TAG,
               "db_write_mavlink_parameter() failed to set new parameter %s ",
               parame_set_payload.param_id);
    }
  } break;
  case FASTMAVLINK_MSG_ID_PARAM_EXT_REQUEST_LIST: {
    ESP_LOGI(
      TAG,
      "GCS requested reading ext parameters list - not supported for now!");
    // ToDo Respond to PARAM_EXT_REQUEST_LIST
  } break;
  case FASTMAVLINK_MSG_ID_PARAM_EXT_REQUEST_READ: {
    ESP_LOGI(TAG,
             "GCS requested reading ext parameter - not supported for now!");
  ...
  case FASTMAVLINK_MSG_ID_COMMAND_LONG: {
    ...
    db_process_mavlink_command(
      &payload, new_msg, fmav_status, buff, origin, tcp_clients, udp_conns);
  } break;
  case FASTMAVLINK_MSG_ID_PING: {
    ...
    uint16_t len = fmav_msg_ping_encode_to_frame_buf(
      buff, db_get_mav_sys_id(), db_get_mav_comp_id(), &payload, fmav_status);
    ESP_LOGD(TAG,
             "Answering MAVLink ping from System %i, Component %i",
             new_msg->sysid,
             new_msg->compid);
    db_route_mavlink_response(buff, len, origin, tcp_clients, udp_conns);
  } break;
  case FASTMAVLINK_MSG_ID_REQUEST_DATA_STREAM: {
    ...
    ESP_LOGW(TAG,
             "GCS requested data stream with ID: %i and rate: %i and "
             "start_stop: %i - ignoring!",
             data_stream_pay.req_stream_id,
             data_stream_pay.req_message_rate,
             data_stream_pay.start_stop);
  } break;
  default: {
    if(new_msg->target_sysid == db_get_mav_sys_id() &&
       new_msg->target_compid == db_get_mav_comp_id()) {
      ESP_LOGW(TAG,
               "Received unknown MAVLink message ID: %u - ignoring",
               new_msg->msgid);
    }
    break;
  }
