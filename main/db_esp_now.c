...
// packet that is filled with payload and sent. Updates the sequ. number with
// every send.
 ...
 *
 * @param password The password that gets transformed to PKCS5 key used for
 * encryption
...
generate_pkcs5_key(const char *password, unsigned char *key, size_t keylen)
{
  ...
  const mbedtls_md_info_t *md_info = mbedtls_md_info_from_type(
    MBEDTLS_MD_SHA256); // Use SHA-256 for key derivation
  ...
  static unsigned char generic_salt[] =
    "GenSalt147894562"; // Generic on purpose. No need for salt in this
                        // application
  ret = mbedtls_pkcs5_pbkdf2_hmac_ext(MBEDTLS_MD_SHA256,
                                      (const unsigned char *)password,
                                      strlen(password),
                                      generic_salt,
                                      strlen((char *)generic_salt),
                                      10000,
                                      keylen,
                                      key);
  switch(ret) {
  ...
  default:
    ESP_LOGW(
      TAG, "mbedtls_pkcs5_pbkdf2_hmac returned unknown error: %02x", ret);
    break;
  }
  // Clean up
                   ...
                   uint8_t encrypt_payload_len)
{
  ...
  // multiple sessions where the password is not changed
  esp_fill_random(db_esp_now_packet->db_esp_now_packet_header.aes_iv,
                  DB_ESPNOW_AES_IV_LEN);
  int ret = mbedtls_gcm_crypt_and_tag(
    &aes,
    MBEDTLS_GCM_ENCRYPT,
    encrypt_payload_len,
    db_esp_now_packet->db_esp_now_packet_header.aes_iv,
    DB_ESPNOW_AES_IV_LEN,
    (uint8_t *)&db_esp_now_packet->db_esp_now_packet_header,
    db_esp_now_packet_header_len,
    (uint8_t *)&db_esp_now_packet->db_esp_now_packet_protected_data,
    (uint8_t *)&db_esp_now_packet->db_esp_now_packet_protected_data,
    DB_ESPNOW_AES_TAG_LEN,
  ...
  if(ret == 0) {
    ...
    if(ret == MBEDTLS_ERR_GCM_BAD_INPUT) {
      ESP_LOGW(TAG,
               "mbedtls_gcm_crypt_and_tag returned MBEDTLS_ERR_GCM_BAD_INPUT");
    }
    else {
                   ...
                   uint8_t *decrypt_out_buffer, uint8_t len_encrypted_data)
{
  ...
  case MBEDTLS_ERR_GCM_AUTH_FAILED:
    ESP_LOGW(TAG,
             "MBEDTLS_ERR_GCM_AUTH_FAILED: Authenticated decryption failed.");
    break;
  case MBEDTLS_ERR_GCM_BAD_INPUT:
    ESP_LOGW(TAG,
             "MBEDTLS_ERR_GCM_BAD_INPUT: Bad input parameters to function.");
    break;
  default:
    ESP_LOGW(TAG, "Unknown mbedtls_gcm_auth_decrypt error %i", ret_decrypt);
    break;
  }
  return ret_decrypt;
}
                            ...
                            const uint8_t payload_length)
{
  ...
  // after encryption the payload_length_decrypted will be unreadable -> keep a
  ...
  ret = db_encrypt_payload(
    ...
    (*db_esp_now_packet)
      ...
      1);
  if(ret == 0) {
    err = esp_now_send(BROADCAST_MAC,
                       (const uint8_t *)db_esp_now_packet,
                       (db_esp_now_packet_header_len + DB_ESPNOW_AES_TAG_LEN +
                        payload_length + 1));
    if(err != ESP_OK) {
      ESP_LOGE(TAG,
               "Error (%s) sending ESP-NOW data - size %i !",
               esp_err_to_name(err),
               (db_esp_now_packet_header_len + DB_ESPNOW_AES_TAG_LEN +
                payload_length + 1));
      return false;
...
db_read_uart_queue_and_send()
{
  ...
  if(db_espnow_send_queue != NULL &&
     xQueueReceive(db_espnow_send_queue, &evt, 0) == pdTRUE) {
    db_esp_now_packet_global.db_esp_now_packet_header.packet_type =
      evt.packet_type;
    if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_ESPNOW_GND) {
      db_esp_now_packet_global.db_esp_now_packet_header.origin =
        DB_ESPNOW_ORIGIN_GND;
    }
    else {
      db_esp_now_packet_global.db_esp_now_packet_header.origin =
        DB_ESPNOW_ORIGIN_AIR;
    }
    // Create an DroneBridge for ESP32 ESP-NOW packet
    db_esp_now_packet_global.db_esp_now_packet_protected_data
      .payload_length_decrypted = evt.data_len;
    // ToDo: Potential to optimize: esp_now_send() does not need an instance of
    // buffer after sending, however using evt.data as pointer resulted in
    // errors
    ...
    if(db_espnow_encrypt_auth_send(&db_esp_now_packet_global, evt.data_len)) {
      ...
      if(db_esp_now_packet_global.db_esp_now_packet_header.seq_num <
         UINT32_MAX) {
        db_esp_now_packet_global.db_esp_now_packet_header
          .seq_num++; // packet is static so just increase number once we sent
                      // it
      }
      else {
        db_esp_now_packet_global.db_esp_now_packet_header.seq_num =
          0; // catch overflow in case someone got crazy
      }
      return true;
    }
    else {
                 ...
                 uint8_t broadcast_peer_mac[6])
{
  ...
  if(esp_now_clients->size ==
     DB_ESPNOW_MAX_BROADCAST_PEERS) { // Check if the list is full
    return -1;                        // Do nothing - list is full already
  }
  else {
    // Add new peer to list - Copy the element data to the end of the array
    db_esp_now_bpeer_info_t db_esp_now_bpeer_info = {
      .gnd_rssi = -127, .last_seq_num = 0, .gnd_rx_lost_packets = 0
    };
    memcpy(db_esp_now_bpeer_info.broadcast_peer_mac,
           broadcast_peer_mac,
           ESP_NOW_ETH_ALEN);
    esp_now_clients->db_esp_now_bpeer_info[esp_now_clients->size] =
      db_esp_now_bpeer_info;
                           ...
                           int8_t rssi)
{
  ...
  uint8_t len_payload =
    data_len - DB_ESPNOW_AES_TAG_LEN - db_esp_now_packet_header_len;
  ...
  if(db_decrypt_payload(db_esp_now_packet, db_decrypted_data, len_payload) ==
    ...
    if(peer_index != -1) {
      if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_ESPNOW_GND) {
        ...
        db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index].gnd_rssi =
          rssi;
      }
      else {
        // AIR unit keeps track of RSSI using db_esp_signal_quality variable
        // no need to do here anything
        // TODO: Clean up RSSI variables - a bit confusing that AIR and GND use
        // different structures
      }
      // check packet sequence number
      if(db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index]
           .last_seq_num <
         db_esp_now_packet->db_esp_now_packet_header.seq_num) {
        // Count the lost packets per peer since the last received packet
        db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index]
          .gnd_rx_lost_packets +=
          (db_esp_now_packet->db_esp_now_packet_header.seq_num -
           db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index]
             .last_seq_num -
        ...
        ESP_LOGW(TAG,
                 "Sequence number lower than expected. Sender did reset or "
                 "packet may be part of a replay attack.");
        // accept packet anyway for now to make for a more robust link
        db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index]
          .gnd_rx_lost_packets = 0;
      }

      db_esp_now_clients_list->db_esp_now_bpeer_info[peer_index].last_seq_num =
        db_esp_now_packet->db_esp_now_packet_header.seq_num;
    }
    else {
      /* Do nothing since we only count packet loss and rssi on the GND side
       * not on the air side and only if there is still a free spot in the list
       */
    }

    /* Process packet depending on packet type */
    if(db_esp_now_packet->db_esp_now_packet_header.packet_type ==
       DB_ESP_NOW_PACKET_TYPE_DATA) {
      // Pass data to UART queue
      db_espnow_queue_event_t db_uart_evt;
      db_uart_evt.data_len =
        db_decrypted_data[0]; // should be equal to len_payload-1 if everything
                              // worked out
      db_uart_evt.data = malloc(db_uart_evt.data_len);
      // For some reason it seems we cannot directly decrypt to
      // db_espnow_uart_event_t -> Queues get set to NULL
      memcpy(db_uart_evt.data, &db_decrypted_data[1], db_uart_evt.data_len);
      if(xQueueSend(db_uart_write_queue, &db_uart_evt, ESPNOW_MAXDELAY) !=
         pdTRUE) {
    ...
    else if(db_esp_now_packet->db_esp_now_packet_header.packet_type ==
            DB_ESP_NOW_PACKET_TYPE_INTERNAL_TELEMETRY) {
      /* This is only called on the AIR side since GND sends telemetry to AIR
       * only */
      db_esp_now_clients_list_t *clients =
        (db_esp_now_clients_list_t *)&db_decrypted_data[1];
      ESP_LOGD(TAG,
               "Received internal telemetry frame containing %i entries",
               clients->size);
      for(int i = 0; i < clients->size; i++) {
        if(memcmp(LOCAL_MAC_ADDRESS,
                  clients->db_esp_now_bpeer_info[i].broadcast_peer_mac,
                  ESP_NOW_ETH_ALEN) == 0) {
          // found us (this local ESP32 AIR unit) -> update internal telemetry
          // buffer, so it gets sent with next Mavlink RADIO STATUS in case
          // MAVLink radio status is enabled
          db_esp_signal_quality.gnd_noise_floor = clients->gnd_noise_floor;
          db_esp_signal_quality.gnd_rssi =
            clients->db_esp_now_bpeer_info[i].gnd_rssi;
          db_esp_signal_quality.gnd_rx_packets_lost =
            clients->db_esp_now_bpeer_info[i].gnd_rx_lost_packets;
          break;
        }
        else {
      ...
      ESP_LOGW(TAG,
               "Received unknown DroneBridge for ESP32 ESP-NOW packet type");
    }
  }
  else {
...
db_espnow_send_callback(const uint8_t *mac_addr, esp_now_send_status_t status)
{
  ...
  if(status == ESP_NOW_SEND_FAIL) {
    ESP_LOGE(TAG,
             "Failed to send ESP-NOW packet - MAC: "
             "%02xh:%02xh:%02xh:%02xh:%02xh:%02xh",
             mac_addr[0],
             mac_addr[1],
             mac_addr[2],
             mac_addr[3],
  ...
  if(xQueueSend(db_espnow_send_recv_callback_queue, &evt, ESPNOW_MAXDELAY) !=
     pdTRUE) {
                           ...
                           const uint8_t *data, int len)
{
  ...
  if(data[0] == DB_ESPNOW_ORIGIN_GND &&
  ...
  defined(CONFIG_IDF_TARGET_ESP32S2) || defined(CONFIG_IDF_TARGET_ESP32C6)
    db_esp_now_clients_list->gnd_noise_floor =
      (int8_t)recv_info->rx_ctrl->noise_floor;
    // rest RSSI will be processed in process_espnow_data()
#endif
  }

  if(data[0] == DB_ESPNOW_ORIGIN_AIR &&
     DB_PARAM_RADIO_MODE == DB_WIFI_MODE_ESPNOW_AIR) {
    // Ignoring packet - Came from another AIR station
    return;
  }
  else {
    // we are AIR a packet is for us from GND - we only expect one GND station
    // to be talking to us
  ...
  if(db_espnow_send_recv_callback_queue != NULL &&
     xQueueSend(db_espnow_send_recv_callback_queue, &evt, ESPNOW_MAXDELAY) !=
       pdTRUE) {
...
init_gcm_encryption_module(uint8_t *aes_key)
{
  ...
  int ret = mbedtls_gcm_setkey(
    &aes, MBEDTLS_CIPHER_ID_AES, aes_key, DB_ESPNOW_AES_KEY_LEN);
  return ret;
}
...
deinit_espnow_all()
{
  ...
  vSemaphoreDelete(
    db_espnow_send_queue); // ToDo: Check if that is a good idea since control
...
db_espnow_init()
{
  ...
  db_espnow_send_recv_callback_queue =
    xQueueCreate(ESPNOW_QUEUE_SIZE, sizeof(db_espnow_event_t));
  if(db_espnow_send_recv_callback_queue == NULL) {
  ...
  db_espnow_send_queue =
    xQueueCreate(ESPNOW_QUEUE_SIZE, sizeof(db_espnow_queue_event_t));
  db_uart_write_queue =
    xQueueCreate(ESPNOW_QUEUE_SIZE, sizeof(db_espnow_queue_event_t));
  if(db_espnow_send_queue == NULL) {
  ...
  ESP_ERROR_CHECK(
    esp_now_set_pmk(aes_key)); // only first 16 bytes will be used

  if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_ESPNOW_GND) {
    ESP_LOGI(
      TAG,
      "ESP-NOW for DroneBridge init done - acting as ESP-NOW GND station");
  }
  else {
    ESP_LOGI(
      TAG,
      "ESP-NOW for DroneBridge init done - acting as ESP-NOW AIR station");
  }
  return ESP_OK;
}
...
db_espnow_schedule_internal_telemetry_packet()
{
  ...
  if(sizeof(db_esp_now_clients_list_t) > DB_ESPNOW_PAYLOAD_MAXSIZE) {
    ESP_LOGE(TAG,
             "Size of db_esp_now_clients_list_t is > %i",
...
process_espnow_data()
{
  ...
  if(err == ESP_OK) {
    ...
    ESP_LOGE(TAG,
             "Failed to init espnow (db_espnow_init()) aborting start of "
             "db_espnow task");
    vTaskDelete(NULL);
  }
  db_espnow_event_t evt;
  // indicator that the last ESP-NOW send callback returned -> we can send the
  // next packet
  bool ready_to_send = true; // initially true
  bool send_internal_telemetry_frame =
    false; // flag that indicates a new internal telemetry frame shall be sent
  int delay_timer_cnt = 0;
  while(1) {
    if(db_espnow_send_recv_callback_queue == NULL)
      ESP_LOGE(TAG, "db_espnow_send_recv_callback_queue is NULL!");
    if(db_espnow_send_recv_callback_queue != NULL &&
       xQueueReceive(db_espnow_send_recv_callback_queue, &evt, 0) == pdTRUE) {
      switch(evt.id) {
      case DB_ESPNOW_SEND_CB: {
        db_espnow_event_send_cb_t *send_cb = &evt.info.send_cb;
        // indicate that we can send next packet - the last sending callback
        // has returned, so we keep the order of packets. ESP-NOW by default
        // does not guarantee the order when many packets are sent in quick
        // succession
        ready_to_send = true;
        if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_ESPNOW_GND &&
    ...
    if(delay_timer_cnt == 5000) {
      ...
      send_internal_telemetry_frame =
        true; // use this timer trigger to schedule a new internal telemetry
              // frame
      delay_timer_cnt = 0;
    }
    else {
