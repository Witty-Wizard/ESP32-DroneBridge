 ...
 *****************************************************************************/
static int db_open_serial_udp_socket();
 ...
 * @return returns socket file descriptor
 *****************************************************************************/
static int db_open_serial_udp_broadcast_socket();
 ...
 * @return returns socket file descriptor
 *****************************************************************************/
static int db_open_int_telemetry_udp_socket();
 ...
 *****************************************************************************/
static void db_send_to_all_udp_clients(udp_conn_list_t *n_udp_conn_list,
 ...
 *****************************************************************************/
static void db_send_to_all_espnow(uint8_t data[], const uint16_t *data_length);
...
static int db_open_serial_udp_socket() {
  ...
  if (udp_socket < 0) {
    ESP_LOGE(TAG, "Unable to create socket for serial communication: errno %d",
             errno);
    return -1;
  }
  int broadcastEnable = 1;
  int err = setsockopt(udp_socket, SOL_SOCKET, SO_BROADCAST, &broadcastEnable,
                       sizeof(broadcastEnable));
  if (err < 0) {
    ESP_LOGE(TAG,
             "Socket unable to set udp socket to accept broadcast messages: "
             "errno %d",
             errno);
  }
  err = bind(udp_socket, (struct sockaddr *)&server_addr, sizeof(server_addr));
  if (err < 0) {
    ESP_LOGE(TAG, "Socket unable to bind to %i errno %d", APP_PORT_PROXY_UDP,
             errno);
  }
...
static int db_open_serial_udp_broadcast_socket() {
  ...
  err = bind(udp_socket, (struct sockaddr *)&server_addr, sizeof(server_addr));
  if (err < 0) {
    ESP_LOGE(TAG, "Socket unable to bind Skybrush socket to %i errno %d",
             UDP_BROADCAST_PORT_SKYBRUSH, errno);
  }
  fcntl(udp_socket, F_SETFL, O_NONBLOCK);
  ESP_LOGI(TAG, "Opened UDP broadcast enabled socket on port %i",
...
static int db_open_int_telemetry_udp_socket() {
  ...
  if (DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA) {
    ...
    ESP_LOGI(TAG, "Configured internal Multicast address %s",
  ...
  ESP_LOGI(TAG, "Opened internal telemetry socket on port: %i",
...
static void db_send_to_all_udp_clients(udp_conn_list_t *n_udp_conn_list,
  ...
  for (int i = 0; i < n_udp_conn_list->size; i++) { // send to all UDP clients
    int sent = sendto(
        n_udp_conn_list->udp_socket, data, data_length, 0,
        (struct sockaddr *)&n_udp_conn_list->db_udp_clients[i].udp_client,
        sizeof(n_udp_conn_list->db_udp_clients[i].udp_client));
    if (sent != data_length) {
      int err = errno;
      ESP_LOGE(TAG, "UDP - Error sending (%i/%i) to because of %d", sent,
               data_length, err);
    }
  }
}

static void db_send_to_all_espnow(uint8_t data[],
 ...
 * information
 */
void read_process_serial_link(int *tcp_clients, uint *transparent_buff_pos,
  ...
  case DB_SERIAL_PROTOCOL_MSPLTM:
    db_parse_msp_ltm(tcp_clients, udp_conn_list, msp_message_buffer,
                     msp_ltm_buff_pos, db_msp_ltm_port);
    break;
  case 3:
  case DB_SERIAL_PROTOCOL_MAVLINK:
    db_read_serial_parse_mavlink(tcp_clients, udp_conn_list,
                                 msp_message_buffer, transparent_buff_pos);
    break;
  case DB_SERIAL_PROTOCOL_TRANSPARENT:
  default:
    db_read_serial_parse_transparent(tcp_clients, udp_conn_list, serial_buffer,
 ...
 * @param sta_list
 */
void db_send_internal_telemetry_to_stations(int tel_sock,
  ...
  if (DB_PARAM_RADIO_MODE == DB_WIFI_MODE_AP_LR && udp_conns->size > 0 &&
    ...
    inet_ntoa_r(((struct sockaddr_in *)res->ai_addr)->sin_addr, addr_buf,
                sizeof(addr_buf) - 1);
    ...
    err = sendto(tel_sock, buffer, buffer_pos + 1, 0, res->ai_addr,
                 res->ai_addrlen);
    freeaddrinfo(res);
    if (err < 0) {
 ...
 */
void handle_internal_telemetry(int tel_sock, uint8_t *udp_buffer,
                               socklen_t *sock_len,
  ...
  if (tel_sock > 0) {
    ...
    if (recv_length > 0) {
      ESP_LOGD(TAG, "Got internal telem. frame containing %i entries",
               udp_buffer[0]);
      for (int i = 1; i < (udp_buffer[0] * 7); i += 7) {
        if (memcmp(LOCAL_MAC_ADDRESS, &udp_buffer[i], ESP_NOW_ETH_ALEN) == 0) {
          ...
          ESP_LOGD(TAG, "AP receives our packets with RSSI: %i",
...
/*****************************************************************************
 * @brief Thread that manages all incoming and outgoing ESP-NOW and serial
 * (UART) connections. Called only when ESP-NOW mode is selected
 *****************************************************************************/
static _Noreturn void control_module_esp_now() {
  ...
  while (1) {
    ...
    read_process_serial_link(NULL, &transparent_buff_pos, &msp_ltm_buff_pos,
                             msp_message_buffer, serial_buffer,
                             &db_msp_ltm_port);
    ...
    if (db_uart_write_queue != NULL &&
      ...
      if (DB_PARAM_SERIAL_PROTO == DB_SERIAL_PROTOCOL_MAVLINK) {
        ...
        db_parse_mavlink_from_radio(NULL, NULL, db_espnow_uart_evt.data,
 ...
 *****************************************************************************/
static _Noreturn void control_module_udp_tcp() {
  ...
  if (serial_socket_status == ESP_FAIL) {
    ESP_LOGE(
        TAG,
        "JTAG serial socket not opened. Aborting start of control module.");
    vTaskDelete(NULL);
  } else {
#ifdef CONFIG_DB_SERIAL_OPTION_JTAG
    db_jtag_serial_info_print();
  ...
  while (1) {
    ...
    for (int i = 0; i < CONFIG_LWIP_MAX_ACTIVE_TCP;
         i++) { // handle TCP clients
      if (tcp_clients[i] > 0) {
        ssize_t recv_length =
        ...
        if (recv_length > 0) {
          if (DB_PARAM_SERIAL_PROTO == DB_SERIAL_PROTOCOL_MAVLINK) {
            ...
            // know when to do so to not "destroy" an existign packet
            db_parse_mavlink_from_radio(tcp_clients, udp_conn_list,
                                        tcp_client_buffer, recv_length);
          } else {
          ...
          ESP_LOGE(TAG, "Error receiving from TCP client %i (fd: %i): %d", i,
                   tcp_clients[i], errno);
          shutdown(tcp_clients[i], 0);
          close(tcp_clients[i]);
          num_connected_tcp_clients--;
    ...
    // UART
    ssize_t recv_length = recvfrom(
        udp_conn_list->udp_socket, udp_buffer, UDP_BUF_SIZE, 0,
    ...
    if (recv_length > 0) {
      if (DB_PARAM_SERIAL_PROTO == DB_SERIAL_PROTOCOL_MAVLINK) {
        ...
        // first since we might inject packets and do not know when to do so to
        // not "destroy" an existing packet
        db_parse_mavlink_from_radio(tcp_clients, udp_conn_list, udp_buffer,
                                    recv_length);
      } else {
        // no parsing with any other protocol - transparent here
      ...
      // connected devices. In AP-Mode the devices can be removed based on the
      // IP/MAC address
    ...
    if (DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA &&
      ...
      recv_length = recvfrom(
          udp_broadcast_skybrush_socket, udp_buffer, UDP_BUF_SIZE, 0,
          (struct sockaddr *)&new_db_udp_client.udp_client, &udp_socklen);
      if (recv_length > 0) {
    ...
    if (DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA) {
      handle_internal_telemetry(db_internal_telem_udp_sock, udp_buffer,
                                &udp_socklen, &new_db_udp_client.udp_client);
    } else {
    ...
    read_process_serial_link(tcp_clients, &transparent_buff_pos,
                             &msp_ltm_buff_pos, msp_message_buffer,
                             serial_buffer, &db_msp_ltm_port);
    ...
    if (delay_timer_cnt == 6000) {
      ...
      if (DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA) {
        ...
        db_send_internal_telemetry_to_stations(db_internal_telem_udp_sock,
                                               &wifi_sta_list, udp_conn_list);
      } else {
  ...
  while (1) {
    ...
    if (db_uart_write_queue_ble != NULL &&
      ...
      if (DB_PARAM_SERIAL_PROTO == DB_SERIAL_PROTOCOL_MAVLINK) {
        ...
        // first since we might inject packets and do not know when to do so to
        // not "destroy" an existing packet
        db_parse_mavlink_from_radio(NULL, NULL, bleData.data,
                                    bleData.data_len);
      } else {
        // no parsing with any other protocol - transparent here - just pass
        // through
...
void db_start_control_module() {
  ...
  case DB_WIFI_MODE_ESPNOW_AIR:
    xTaskCreate(&control_module_esp_now, /**< Task function for ESP-NOW
                                            communication */
  ...
  case DB_BLUETOOTH_MODE:
    xTaskCreate(&control_module_ble, /**< Task function for Bluetooth SPP
                                        communication */
  ...
  default:
    xTaskCreate(&control_module_udp_tcp, /**< Task function for UDP/TCP
                                            communication */
...
bool add_to_known_udp_clients(udp_conn_list_t *n_udp_conn_list,
  ...
  inet_ntop(AF_INET, &(new_db_udp_client.udp_client.sin_addr), ip_string,
            INET_ADDRSTRLEN);
  sprintf(ip_port_string, "%s:%d", ip_string,
          htons(new_db_udp_client.udp_client.sin_port));
  ESP_LOGI(TAG, "Added %s to udp client distribution list - save to NVM: %i",
           ip_port_string, save_to_nvm);
  // save to memory
  if (save_to_nvm) {
...
void db_send_to_all_clients(int tcp_clients[],
                            udp_conn_list_t *n_udp_conn_list, uint8_t data[],
  ...
  case DB_BLUETOOTH_MODE:
    ...
    if (xQueueSend(db_uart_read_queue_ble, &bleData, portMAX_DELAY) !=
        pdPASS) {
...
bool remove_from_known_udp_clients(udp_conn_list_t *n_udp_conn_list,
                                   struct db_udp_client_t new_db_udp_client) {
  if (n_udp_conn_list == NULL) { // Check if the list is NULL
    return false;                // Do nothing
  }
    ...
    if (memcmp(n_udp_conn_list->db_udp_clients[i].mac, new_db_udp_client.mac,
      ...
      for (int j = i; j < n_udp_conn_list->size - 1;
        ...
        n_udp_conn_list->db_udp_clients[j] =
            n_udp_conn_list->db_udp_clients[j + 1]; // Shift the array elements
                                                    // to the left
      }
