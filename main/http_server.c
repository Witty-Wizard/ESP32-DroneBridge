...
settings_post_handler(httpd_req_t *req)
{
  ...
  httpd_resp_sendstr(req,
                     ...
                     "  }");
  vTaskDelay(1000 / portTICK_PERIOD_MS); // wait to allow the website
  ...
  vTaskDelay(500 / portTICK_PERIOD_MS); // wait 1s to allow the website
                                        // displaying the success message
  esp_restart();
  return ESP_OK;
}
...
/**
 * Process incoming UDP connection add request. Only one IPv4 connection can be
 * added at a time Expecting JSON in the form of:
 * {
 *   "ip": "XXX.XXX.XXX.XXX",
 *   "port": 452
 * }
 ...
 */
static esp_err_t
settings_clients_udp_post(httpd_req_t *req)
{
  int total_len = req->content_len;
  int cur_len = 0;
  char *buf = ((rest_server_context_t *)(req->user_ctx))->scratch;
  ...
  cJSON *json =
    cJSON_GetObjectItem(root, (char *)db_param_udp_client_ip.db_name);
  if(json)
 ...
 * ESP32 will remove all maintained UDP connections from its internal list and
 * UDP clients will have to register again. This also clears the one UDP client
 * connection that gets saved to NVM.
 *
 * @param req
 * @return ESP_OK if all was good. Else ESP_FAIL
 */
static esp_err_t
settings_clients_clear_udp_get(httpd_req_t *req)
{
  for(int i = 0; i < udp_conn_list->size; ++i) {
    memset(
      &udp_conn_list->db_udp_clients[i], 0, sizeof(struct db_udp_client_t));
  }
...
/**
 * Returns a JSON containing read bytes from UART, number of connected TCP
 * connections and UDP broadcasts as well as the current IP address of the
 * ESP32
 * @param req
 * @return ESP_OK on successfully sending the http request
 */
static esp_err_t
system_stats_get_handler(httpd_req_t *req)
{
  httpd_resp_set_type(req, "application/json");
  cJSON *root = cJSON_CreateObject();
  ...
  for(int i = 0; i < udp_conn_list->size; i++) {
    ...
    inet_ntop(AF_INET,
              &(udp_conn_list->db_udp_clients[i].udp_client.sin_addr),
              ip_string,
              INET_ADDRSTRLEN);
    sprintf(ip_port_string,
            "%s:%d",
            ip_string,
            htons(udp_conn_list->db_udp_clients[i].udp_client.sin_port));
  ...
  if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA) {
    ...
    for(int i = 0; i < wifi_sta_list.num; i++) {
      ...
      cJSON_AddNumberToObject(
        connected_stations_status, "sta_rssi", wifi_sta_list.sta[i].rssi);
 ...
 */
static esp_err_t
system_clients_get_handler(httpd_req_t *req)
{
  httpd_resp_set_type(req, "application/json");
  cJSON *root = cJSON_CreateObject();
  //    cJSON *tcp_clients = cJSON_CreateArray();
  //    for (int i = 0; i < udp_conn_list->size; i++) {
  //        //TODO: Save the TCP IPs and port during accept and put them here
  //        into a JSON
  //    }
...
start_rest_server(const char *base_path)
{
  ...
  REST_CHECK(
    httpd_start(&server, &config) == ESP_OK, "Start server failed", err_start);

  /* URI handler for fetching system info */
  httpd_uri_t system_info_get_uri = { .uri = "/api/system/info",
                                      .method = HTTP_GET,
  ...
  httpd_uri_t system_clients_get_uri = { .uri = "/api/system/clients",
                                         .method = HTTP_GET,
  ...
  /* URI handler for fetching system info */
  httpd_uri_t system_stats_get_uri = { .uri = "/api/system/stats",
                                       .method = HTTP_GET,
  ...
  httpd_uri_t settings_clients_udp_post_uri = { .uri =
                                                  "/api/settings/clients/udp",
                                                .method = HTTP_POST,
                                                .handler =
                                                  settings_clients_udp_post,
                                                .user_ctx = rest_context };
