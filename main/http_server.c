 ...
 * connections and UDP broadcasts as well as the current IP address of the ESP32
 * @param req
...
static esp_err_t settings_post_handler(httpd_req_t *req) {
  ...
  httpd_resp_sendstr(req, "{\n"
                          ...
                          "  }");
  vTaskDelay(1000 / portTICK_PERIOD_MS); // wait to allow the website displaying
                                         // the success message
  // Send reboot message
...
static esp_err_t settings_clients_clear_udp_get(httpd_req_t *req) {
  for (int i = 0; i < udp_conn_list->size; ++i) {
    memset(&udp_conn_list->db_udp_clients[i], 0,
           sizeof(struct db_udp_client_t));
  }
...
static esp_err_t system_info_get_handler(httpd_req_t *req) {
  ...
  char mac_str[18];
  sprintf(mac_str, "%02X:%02X:%02X:%02X:%02X:%02X", LOCAL_MAC_ADDRESS[0],
          LOCAL_MAC_ADDRESS[1], LOCAL_MAC_ADDRESS[2], LOCAL_MAC_ADDRESS[3],
          LOCAL_MAC_ADDRESS[4], LOCAL_MAC_ADDRESS[5]);
...
static esp_err_t system_stats_get_handler(httpd_req_t *req) {
  ...
  for (int i = 0; i < udp_conn_list->size; i++) {
    ...
    inet_ntop(AF_INET, &(udp_conn_list->db_udp_clients[i].udp_client.sin_addr),
  ...
  if (DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA) {
    ...
    for (int i = 0; i < wifi_sta_list.num; i++) {
      ...
      cJSON_AddNumberToObject(connected_stations_status, "sta_rssi",
