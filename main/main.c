...
static esp_err_t
db_set_dns_server(esp_netif_t *netif, uint32_t addr, esp_netif_dns_type_t type)
{
...
set_client_static_ip()
{
  if(DB_PARAM_RADIO_MODE == DB_WIFI_MODE_STA &&
    ...
    ESP_LOGD(TAG,
             "Success to set static ip: %s, netmask: %s, gw: %s",
             (char *)DB_PARAM_STA_IP,
             (char *)DB_PARAM_STA_IP_NETMASK,
             (char *)DB_PARAM_STA_GW);
    ESP_ERROR_CHECK(db_set_dns_server(esp_default_netif,
                                      ...
                                      ESP_NETIF_DNS_MAIN));
    ESP_ERROR_CHECK(db_set_dns_server(
      esp_default_netif, ipaddr_addr("0.0.0.0"), ESP_NETIF_DNS_BACKUP));
  }
  else {
                   ...
                   void *event_data)
{
  ...
  if(event_id == WIFI_EVENT_AP_STACONNECTED) {
    ...
    ESP_LOGI(TAG,
             "WIFI_EVENT - Client connected - station:" MACSTR ", AID=%d",
             MAC2STR(event->mac),
             event->aid);
    ...
    ESP_LOGI(TAG,
             "WIFI_EVENT_AP_START (SSID: %s PASS: %s)",
             DB_PARAM_WIFI_SSID,
             DB_PARAM_PASS);
  }
    ...
    ESP_LOGI(TAG,
             "IP_EVENT_AP_STAIPASSIGNED - New station IP:" IPSTR,
             IP2STR(&event->ip));
    ESP_LOGI(
      TAG, "IP_EVENT_AP_STAIPASSIGNED - MAC: " MACSTR, MAC2STR(event->mac));
    struct db_udp_client_t db_udp_client;
  ...
  if(event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    ...
    if(!DB_RADIO_IS_OFF) { // maybe the other task did set it in the meantime
      ...
      ESP_LOGW(TAG,
               "Did not start Wi-Fi since autopilot told us he is armed "
    ...
    if(!DB_RADIO_IS_OFF) {
      ...
      ESP_LOGD(TAG,
               "WIFI_EVENT_STA_DISCONNECTED - did not try to re-connect since "
               "WiFi was commanded off");
    }
  }
...
init_fs(void)
{
  ...
  if(ret != ESP_OK) {
    ESP_LOGE(
      TAG, "Failed to register semihost driver (%s)!", esp_err_to_name(ret));
    return ESP_FAIL;
  }
  return ESP_OK;
}
...
init_fs(void)
{
  ...
  if(ret != ESP_OK) {
    ...
    ESP_LOGI(TAG,
             "Filesystem init finished! Partition size: total: %d bytes, "
             "used: %d bytes (%i%%)",
             total,
             used,
             (used * 100) / total);
 ...
 */
void
db_init_wifi_apmode(int wifi_mode)
{
...
#pragma GCC diagnostic pop
  ...
  if(wifi_mode == DB_WIFI_MODE_AP_LR) {
    ESP_LOGI(TAG,
             "Enabling LR Mode on access point. This device will be invisible "
             "to non-ESP32 devices!");
    ESP_ERROR_CHECK(
...
#pragma GCC diagnostic pop
  ...
  ESP_LOGI(TAG,
           "Init of WiFi Client-Mode finished. (SSID: %s PASS: %s)",
           DB_PARAM_WIFI_SSID,
           DB_PARAM_PASS);
  ...
  if(enable_temp_ap_mode) {
    ESP_LOGW(TAG,
             "WiFi client mode was not able to connect to the specified "
             "access point");
    return -1;
  }
...
db_init_wifi_espnow()
{
  ...
  ESP_LOGI(TAG,
           "Enabled ESP-NOW WiFi Mode! LR Mode is set. This device will be "
 ...
 */
void
db_set_radio_status(uint8_t enable_wifi)
{
 ...
 */
void
save_udp_client_to_nvm(struct db_udp_client_t *new_db_udp_client,
                       bool clear_client)
{
  ...
  if(!clear_client) {
    ...
    inet_ntop(
      AF_INET, &(new_db_udp_client->udp_client.sin_addr), ip, INET_ADDRSTRLEN);
...
db_read_settings_nvs()
{
  ...
  if(nvs_open(NVS_NAMESPACE, NVS_READONLY, &my_handle) != ESP_OK) {
    ESP_LOGI(
      TAG, "NVS namespace not found. Using default values, setting up NVS...");
    nvs_close(my_handle);
    ...
    db_param_print_values_to_buffer(param_str_buffer);
    ESP_LOGI(
      TAG, "Initialized with default values:\n%s", (char *)param_str_buffer);
  }
  else {
    ...
    if(strlen((char *)db_param_udp_client_ip.value.db_param_str.value) > 0 &&
      ...
      ESP_LOGI(TAG,
               "Adding %s:%i to known UDP clients.",
 ...
 */
void
short_press_callback(void *arg, void *usr_data)
{
  ESP_LOGW(TAG,
           "Short press detected setting wifi mode to access point with "
           "password: dronebridge");
 ...
 */
void
long_press_callback(void *arg, void *usr_data)
{
  ESP_LOGW(TAG,
           "Reset triggered via GPIO %i. Resetting settings and rebooting",
           DB_RESET_PIN);
  DB_RADIO_MODE_DESIGNATED =
    DB_WIFI_MODE_AP; // do not directly change DB_PARAM_RADIO_MODE since it is
                     // not safe and constantly processed by other tasks. Save
...
/**
 * Setup boot button GPIO to reset entire ESP32 settings and to force a reboot
 * of the system
 */
void
set_reset_trigger()
{
  ...
  if(NULL == gpio_btn) {
    ...
    iot_button_register_cb(
      gpio_btn, BUTTON_SINGLE_CLICK, short_press_callback, NULL);
    iot_button_register_cb(
      gpio_btn, BUTTON_LONG_PRESS_UP, long_press_callback, NULL);
  }
}
...
app_main()
{
  ...
  case DB_BLUETOOTH_MODE:
    ...
    ESP_LOGE(TAG,
             "Bluetooth is not enabled with this build. Please enable it in "
             "menuconfig and re-compile. Switching to AP mode.");
  ...
  if(DB_PARAM_RADIO_MODE != DB_WIFI_MODE_ESPNOW_AIR &&
    ...
    // Disable legacy support for DroneBridge communication module - no use
    // case for DroneBridge for ESP32 communication_module();
  }
